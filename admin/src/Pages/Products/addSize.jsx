...
const AddSize = () => {
    ...
    const handleRowCheckboxChange = (item) => {
        const isProductSizeSelected = selectedRows.includes(item._id);

        const newSelectedRows = isProductSizeSelected
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await fetchDataFromApi('/api/product/productSize/get-all-productSize');
                setData(res?.data || []);
            } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const result = await toast.promise(
                postData('/api/product/productSize/create', formFields),
                {
                    loading: 'Adding product size... Please wait.',
    ...
    const handleEditProductSize = (productSizeId, productSizeName) => {
        setSizeId(productSizeId);
        setFormFields({ name: productSizeName });
    };

    const handleUpdateProductSize = async (e) => {
        e.preventDefault();
        ...
        try {
            const result = await toast.promise(
                editData(`/api/product/productSize/updateProductSize/${sizeId}`, {
                    ...formFields,
                    userId: context?.userData?._id,
                    productSizeId: sizeId,
                }),
                {
                    loading: 'Updating product size... Please wait.',
    ...
    const handleDeleteProductSize = async (e, id) => {
        e.preventDefault();
        setIsLoading(true);
        ...
        try {
            const result = await toast.promise(
                ...
                {
                    loading: 'Deleting product size... Please wait.',
    ...
    const handleDeleteSelectedRow = async (e, selectedRows) => {
        ...
        try {
            ...
            const result = await toast.promise(
                deleteMultipleData(`/api/product/productSize/delete-multiple-productSize?ids=${idsQueryParam}`),
                {
                    loading: "Deleting product size(s)... Please wait.",
                    success: (response) => {  // Removed async from here
                        if (response.success) {
                            // Update UI to remove the deleted product size
                            setData((prevData) =>
                            ...
                            return response.message || "Product size(s) deleted successfully!";
                        } else {
                            throw new Error(response.message || "An unexpected error occurred.");
                        }
                    },
                    error: (err) => {
                        return err.message || "Failed to delete product size(s). Please try again.";
                    },
                }
            );
    ...
    return (
        <>
            <div className='flex items-center justify-between px-5 pt-3'>
                <h2 className='text-[20px] font-bold'>Product Size</h2>
            </div>

            <div className='card my-4 bg-white border rounded-md px-1 pt-1'>
                <form onSubmit={sizeId ? handleUpdateProductSize : handleSubmit} className='form py-3'>
                        ...
                        <div className='grid grid-cols-1'>
                            <div className='col'>
                                <div className='flex items-center gap-2 mb-1'>
                                    <h3 className='text-[14px] font-medium text-gray-700'>Product Size</h3>
                                    <span className='text-gray-400 text-[14px]'>(for eg. S, M, L, XL,...)</span>
                                </div>
                                <input
                                    type='text'
                                ...
                                <div className='flex items-center gap-4'>
                                    ...
                                    <Button
                                        type='submit'
                                        className={`!capitalize w-full sm:w-[150px] !px-5 h-[40px] flex items-center justify-center gap-2 ${sizeId ? (isLoading ? "custom-btn-update-disabled" : "custom-btn-update") : (isLoading ? "custom-btn-disabled" : "custom-btn")}`}
                                        disabled={isLoading}
                                    >
                                            ...
                                            <TableCell scope="row" className="px-6 py-3 text-left">
                                                <div className="flex items-center gap-2">
                                                    <Tooltip title="Edit Product" arrow placement="top">
                                                        <Button className="!h-[35px] !w-[35px] !min-w-[35px] !bg-blue-500 !text-white shadow" onClick={() => handleEditProductSize(item?._id, item?.name)}>
                                                            <MdOutlineEdit className="text-[35px]" />
                                                        </Button>
                                                    </Tooltip>
                                                    <Tooltip title="Delete Product" arrow placement="top">
                                                        <Button className="!h-[35px] !w-[35px] !min-w-[35px] !bg-red-500 !text-white shadow" onClick={(e) => handleDeleteProductSize(e, item?._id)}>
